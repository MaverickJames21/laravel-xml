...
class EmployeeService {
    ...
    public function readXML() {
        try {
            /*
            // $xml = new \SimpleXMLElement(public_path("streha_invoice_03.xml"));
            $xmldata = simplexml_load_file(public_path("streha_invoice_03.xml"));
            $data = new \SimpleXMLElement(public_path("streha_invoice_03.xml"), 0, TRUE);
            var_dump($data);
            */

            // Use Dom methode to get data
            $dom = new \DomDocument("1.0", "ISO-8859-1");
            $dom->load(public_path("streha_invoice_03.xml"));
